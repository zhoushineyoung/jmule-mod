...
public class SearchTab extends MainTab {
	...
	private void search() {
		if ((searchType.getSelectionIndex()==0)&&(!_core.getServerManager().isConnected())) {
			Utils.showWarningMessage(tab_content.getShell(), Localizer._("mainwindow.searchtab.not_connected_to_server_title"), Localizer._("mainwindow.searchtab.not_connected_to_server"));
			return;
		}
		if ((searchType.getSelectionIndex()==1)&&(!_core.getJKadManager().isConnected())) {
			Utils.showWarningMessage(tab_content.getShell(), _._("mainwindow.searchtab.not_connected_to_kad_title"),_._("mainwindow.searchtab.not_connected_to_kad"));
			return;
		}		
